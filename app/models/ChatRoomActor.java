...
public class ChatRoomActor extends UntypedActor {
	...
	static ActorRef defaultRoom = Akka.system().actorOf(
			Props.create(ChatRoomActor.class));
	...
	static {
		...
		Akka.system()
				...
				.scheduleOnce(Duration.create(10, TimeUnit.MILLISECONDS),
						new Runnable() {
							public void run() {
								Jedis j = play.Play.application()
										.plugin(RedisPlugin.class).jedisPool()
										.getResource();
								j.subscribe(new ChatListener(), CHANNEL);
							}
	...
	public static void join(final String username, WebSocket.In<JsonNode> in,
		...
		// Join the default room. Timeout should be longer than the Redis
		// connect timeout (2 seconds)
		String result = (String) Await.result(
				ask(defaultRoom, new Join(username, out), 3000),
		...
		if ("OK".equals(result)) {
			...
			in.onMessage(new Callback<JsonNode>() {
				public void invoke(JsonNode event) {
					...
					Jedis j = play.Play.application().plugin(RedisPlugin.class)
							.jedisPool().getResource();
					try {
						// All messages are pushed through the pub/sub channel
						j.publish(ChatRoomActor.CHANNEL,
								Json.stringify(Json.toJson(talk)));
					} finally {
						play.Play.application().plugin(RedisPlugin.class)
	...
	public void onReceive(Object message) throws Exception {
		...
		Jedis j = play.Play.application().plugin(RedisPlugin.class).jedisPool()
				.getResource();
		...
		try {
			if (message instanceof Join) {
				...
				if (j.sismember(MEMBERS, join.username)) {
					getSender()
							.tell("This username is already used", getSelf());
				} else {
					...
					RosterNotification rosterNotify = new RosterNotification(
							join.username, "join");
					j.publish(ChatRoomActor.CHANNEL,
							Json.stringify(Json.toJson(rosterNotify)));
				...
				if ("join".equals(rosterNotify.direction)) {
					notifyAll("join", rosterNotify.username,
							"has entered the room");
				} else if ("quit".equals(rosterNotify.direction)) {
					notifyAll("quit", rosterNotify.username,
							"has left the room");
				}
			...
			play.Play.application().plugin(RedisPlugin.class).jedisPool()
					.returnResource(j);
		}
	}
	...
	public void notifyAll(String kind, String user, String text) {
		for (WebSocket.Out<JsonNode> channel : members.values()) {
			...
			// Go to Redis to read the full roster of members. Push it down with
			// the message.
			Jedis j = play.Play.application().plugin(RedisPlugin.class)
					.jedisPool().getResource();
			try {
	...
	public static class ChatListener extends JedisPubSub {
		@Override
		public void onMessage(String channel, String messageBody) {
			...
			if ("talk".equals(messageType)) {
				message = new Talk(parsedMessage.get("username").asText(),
						parsedMessage.get("text").asText());
			} else if ("rosterNotify".equals(messageType)) {
				message = new RosterNotification(parsedMessage.get("username")
						.asText(), parsedMessage.get("direction").asText());
			} else if ("quit".equals(messageType)) {
				message = new Quit(parsedMessage.get("username").asText());
			}
			ChatRoomActor.remoteMessage(message);
		}
